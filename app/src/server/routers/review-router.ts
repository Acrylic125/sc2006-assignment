...
import { db } from "@/db";
import { reviewTable, reviewImagesTable, poiTable, itineraryTable, itineraryPOITable } from "@/db/schema";
  ...
  getReviews: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const reviewImages = await db
        ...
        .where(
          inArray(reviewImagesTable.reviewId, reviews.map(r => r.id))
        );
  ...
  createReview: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (existingReview.length > 0) {
        throw new TRPCError({
          code: "CONFLICT",
          message: "You have already reviewed this POI. You can only submit one review per POI.",
        });
      }
