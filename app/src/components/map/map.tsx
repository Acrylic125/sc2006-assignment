...
function useExploreMap(map: mapboxgl.Map | null, enabled: boolean) {
  ...
  useEffect(() => {
    ...
    // Build a map of POI id to checked status from itinerary
    const checkedPOIs = new Map<number, boolean>();
    for (const poi of itinerariesQuery.data?.pois ?? []) {
      checkedPOIs.set(poi.id, poi.checked ?? false);
    }
    ...
    const load = async () => {
      ...
      for (const poi of poisQuery.data) {
        // If POI is in itinerary and checked, show blue; if in itinerary and unchecked, show green; else blue
        let color = "blue";
        if (checkedPOIs.has(poi.id)) {
          color = checkedPOIs.get(poi.id) ? "blue" : "green";
        }
        features.push({
